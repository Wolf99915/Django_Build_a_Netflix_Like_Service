...
class VideoModelTestCase(TestCase):
    def setUp(self):
        ...
        self.boj_b = Video.objects.create(title = 'This is my title', 
            state = PublishStateOption.PUBLISH, video_id='abcasd')
    ...
    def test_draft_case(self):
        qs = Video.objects.filter(state=PublishStateOption.DRAFT)
        self.assertEqual(qs.count(), 1)

    def test_publish_case(self):
        qs = Video.objects.filter(state=PublishStateOption.PUBLISH)
        now = timezone.now()
